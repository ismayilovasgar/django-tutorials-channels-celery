...
class SendNotifcationForm(forms.Form):
...
class NotificationAdmin(admin.ModelAdmin):
    ...
    def add_view(self, request, form_url="", extrac_context=None):
        if request.method == "POST":
            form = SendNotifcationForm(request.POST)
            if form.is_valid():
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_url = [path("send-notification/", self.admin_site.admin_view(self.add_view), name="send-notification"),
        ]
